...
fn handle_modified_unit(
        ...
        || unit.ends_with(".slice")
    {
        ...
        if parse_systemd_bool(new_unit_info, "Service", "X-ReloadIfChanged", false)
            && !units_to_restart.contains(unit)
            && !(if use_restart_as_stop_and_start {
                units_to_restart.contains(unit)
            } else {
                units_to_stop.contains(unit)
            })
        {
            units_to_reload.insert(unit.to_string());
            record_unit(RELOAD_LIST_FILE, unit);
            ...
            || parse_systemd_bool(new_unit_info, "Unit", "X-OnlyManualStart", false)
        {
                ...
                || !unit.ends_with(".service")
            {
                ...
                if units_to_reload.contains(unit) {
...
"#
            ...
            && (unit_state.state == "active" || unit_state.state == "activating")
        {
                ...
                .unwrap_or(true)
            {
                ...
                match compare_units(&current_unit_info, &new_unit_info) {
                    ...
                    UnitComparison::UnequalNeedsReload if !units_to_restart.contains(unit) => {
    ...
    let mut units_to_stop_filtered = units_to_stop.difference(&units_to_filter).peekable();

    // Show dry-run actions.
    if *action == Action::DryActivate {
        if units_to_stop_filtered.peek().is_some() {
            let mut units: Vec<_> = units_to_stop_filtered.map(String::as_str).collect();
            units.sort_by_key(|name| name.to_lowercase());
            eprintln!("would stop the following units: {}", units.join(", "));
        }

        if !units_to_skip.is_empty() {
            let mut units: Vec<_> = units_to_skip.iter().map(String::as_str).collect();
            units.sort_by_key(|name| name.to_lowercase());
            eprintln!(
        ...
        if !units_to_reload.is_empty() {
            let mut units: Vec<_> = units_to_reload.iter().map(String::as_str).collect();
            units.sort_by_key(|name| name.to_lowercase());
            eprintln!("would reload the following units: {}", units.join(", "));
        }

        if !units_to_restart.is_empty() {
            let mut units: Vec<_> = units_to_restart.iter().map(String::as_str).collect();
            units.sort_by_key(|name| name.to_lowercase());
            eprintln!("would restart the following units: {}", units.join(", "));
        }

        let mut units_to_start_filtered = units_to_start.difference(&units_to_filter).peekable();
        if units_to_start_filtered.peek().is_some() {
            let mut units: Vec<_> = units_to_start_filtered.map(String::as_str).collect();
            units.sort_by_key(|name| name.to_lowercase());
    ...
    if !units_to_reload.is_empty() {
        for unit in units_to_reload.clone() {
